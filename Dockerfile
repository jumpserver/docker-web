...
RUN set -e \
    ...
    && wget -qO /opt/download/JumpServer-Video-Player.dmg https://download.jumpserver.org/public/JumpServer-Video-Player-${VIDEO_PLAYER_VERSION}.run \
    && wget -qO /opt/download/JumpServer-Video-Player.exe https://download.jumpserver.org/public/JumpServer-Video-Player-${VIDEO_PLAYER_VERSION}.run \
